...
$indices = foreach ($pIndex in (1..$pairs.Count)) {
    $lRaw,$rRaw = $pairs[$pIndex] -split "`r`n"
...
Function Check-IsDigit {
    ...
    Process {
        If (($CharacterCode -ge 48) -and ($CharacterCode -le 58)) {
            $True
        } Else {
            $False
        }
