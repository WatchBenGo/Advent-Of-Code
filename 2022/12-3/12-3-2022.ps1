...
ForEach ($Backpack in $Data) {
    $Length = $Backpack.Length
    $Middle = $Length /2
    $SideA = ($Backpack.Substring(0,$Middle)).ToCharArray()
    $SideB = ($Backpack.Substring($Middle)).ToCharArray()

    $Matches = @()
    ForEach ($AChar in $SideA) {
        If ($SideB -ccontains $AChar) {
            $Matches += $AChar
        }
    }
